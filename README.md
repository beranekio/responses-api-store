...
**Background transition conflicts:** `ClaimBackgroundResponse`, `CompleteBackgroundResponse`, and `FailBackgroundResponse` return gRPC `FAILED_PRECONDITION` when the stored record is missing, terminal (`cancelled`/`deleted`/`completed`/`failed`), or in the wrong state (for example completing while still `queued`). Rust clients can use `responses_api_store_client::is_failed_precondition`; the Go client provides `client.IsFailedPrecondition(err)`. `ClaimBackgroundResponse` keeps `pending_upstream_request` and `upstream_authorization` on the stored record until completion or failure so stream/autoclaim retries can re-hydrate claim payload after worker crashes.
